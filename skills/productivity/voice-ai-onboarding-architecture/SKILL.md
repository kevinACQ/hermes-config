...
- Transcript archive reference: `references/hermes-retell-transcript-archive.md`. When Kevin explicitly wants full Retell call memory, save raw transcript + raw JSON into GBrain markdown first, then create structured session pages and only promote approved/high-confidence facts to compiled memory. For immediate after-call archival, use the event-triggered local archive pattern in that reference: Retell/Apps Script forward to a tiny local `/retell-archive` server behind a tunnel + shared secret, verified locally before wiring the cloud trigger.
...
- Apps Script / Sheets cannot directly write to Kevin's local `~/.hermes/brain`; prefer local polling/fetching of Retell calls over exposing Kevin's Mac as a public webhook target.
