...
- Transcript archive reference: `references/hermes-retell-transcript-archive.md`. When Kevin explicitly wants full Retell call memory, save raw transcript + raw JSON into GBrain markdown first, then create structured session pages and only promote approved/high-confidence facts to compiled memory.
