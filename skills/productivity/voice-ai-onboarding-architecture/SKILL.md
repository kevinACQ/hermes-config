...
- Native Hermes phone-channel reference: `references/retell-hermes-native-channel-adapter.md`. Use this when the goal is true Desktop/CLI/Discord/Telegram parity: Retell becomes the audio transport, while the real Hermes API Server Sessions runtime owns tools, canonical sessions, memory, execution, and receipts. Do not mistake larger prompt snapshots for channel parity.
...
- Outbound simulation reference: `references/retell-outbound-roleplay-calls.md`. Use it for realistic reservation/vendor/sales role-play calls to Kevin's private test numbers: research the real business first, use a temporary LLM/agent when the production `begin_message` would break the scenario, verify the live call independently, and delete temporary Retell resources after the call ends.
