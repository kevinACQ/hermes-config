...
```bash
bash "${HERMES_HOME:-$HOME/.hermes}/skills/operations/specialized-platform-operations/scripts/touchdesigner-mcp/setup.sh"
```
...
The table above covers the 32 tools used in typical creative workflows. The remaining 4 tools (`td_project_quit`, `td_test_session`, `td_dev_log`, `td_clear_dev_log`) are admin/dev-mode utilities — see `references/touchdesigner-mcp/mcp-tools.md` for the full 36-tool reference with complete parameter schemas.
...
See `references/touchdesigner-mcp/network-patterns.md` for complete build scripts + shader code.
...
| File | What |
|------|------|
| `references/touchdesigner-mcp/pitfalls.md` | Hard-won lessons from real sessions |
| `references/touchdesigner-mcp/operators.md` | All operator families with params and use cases |
| `references/touchdesigner-mcp/network-patterns.md` | Recipes: audio-reactive, generative, GLSL, instancing |
| `references/touchdesigner-mcp/mcp-tools.md` | Full twozero MCP tool parameter schemas |
| `references/touchdesigner-mcp/python-api.md` | TD Python: op(), scripting, extensions |
| `references/touchdesigner-mcp/troubleshooting.md` | Connection diagnostics, debugging |
| `references/touchdesigner-mcp/glsl.md` | GLSL uniforms, built-in functions, shader templates |
| `references/touchdesigner-mcp/postfx.md` | Post-FX: bloom, CRT, chromatic aberration, feedback glow |
| `references/touchdesigner-mcp/layout-compositor.md` | HUD layout patterns, panel grids, BSP-style layouts |
| `references/touchdesigner-mcp/operator-tips.md` | Wireframe rendering, feedback TOP setup |
| `references/touchdesigner-mcp/geometry-comp.md` | Geometry COMP: instancing, POP vs SOP, morphing |
| `references/touchdesigner-mcp/audio-reactive.md` | Audio band extraction, beat detection, envelope following |
| `references/touchdesigner-mcp/animation.md` | LFOs, timers, keyframes, easing, expression-driven motion |
| `references/touchdesigner-mcp/midi-osc.md` | MIDI/OSC controllers, TouchOSC, multi-machine sync |
| `references/touchdesigner-mcp/particles.md` | POPs and legacy particleSOP — emission, forces, collisions |
| `references/touchdesigner-mcp/projection-mapping.md` | Multi-window output, corner pin, mesh warp, edge blending |
| `references/touchdesigner-mcp/external-data.md` | HTTP, WebSocket, MQTT, Serial, TCP, webserverDAT |
| `references/touchdesigner-mcp/panel-ui.md` | Custom params, panel COMPs, button/slider/field, panelExecuteDAT |
| `references/touchdesigner-mcp/replicator.md` | replicatorCOMP — data-driven cloning, layouts, callbacks |
| `references/touchdesigner-mcp/dat-scripting.md` | Execute DAT family — chop/dat/parameter/panel/op/executeDAT |
| `references/touchdesigner-mcp/3d-scene.md` | Lighting rigs, shadows, IBL/cubemaps, multi-camera, PBR |
| `scripts/touchdesigner-mcp/setup.sh` | Automated setup script |
