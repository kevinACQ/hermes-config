...
# Gstack Openclaw Retro Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `gstack-openclaw-retro`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/gstack-openclaw-retro <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/gstack-openclaw-retro/SKILL.md`
...
4. Preserve any user instruction supplied after `/gstack-openclaw-retro` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/gstack-openclaw-retro`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/gstack-openclaw-retro`
