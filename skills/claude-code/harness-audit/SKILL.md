---
name: harness-audit
description: Audits ~/.claude/CLAUDE.md for pointer purity and hook integrity. Computes a 0-10 score, classifies every line as behavioral/pointer/fat, and tracks trend history. Does not edit the harness.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Harness Audit Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `harness-audit`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/harness-audit <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/harness-audit/SKILL.md`
...
4. Preserve any user instruction supplied after `/harness-audit` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/harness-audit`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/harness-audit`
