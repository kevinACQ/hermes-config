---
name: skool-post-create
description: Create a new post in any Skool community. Handles category selection, duplicate detection, and human review gate before posting. Use for announcements, training reminders, or any new Skool content.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Skool Post Create Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `skool-post-create`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/skool-post-create <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-post-create/SKILL.md`
...
4. Preserve any user instruction supplied after `/skool-post-create` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-post-create`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-post-create`
