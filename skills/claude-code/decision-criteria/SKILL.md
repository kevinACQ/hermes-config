...
description: Observable ROI formula for ranking solution options. Use when comparing approaches in /office-hours, /autoplan, or any design decision. Both Kevin and Claude can independently measure and verify every input.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Decision Criteria Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `decision-criteria`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/decision-criteria <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/decision-criteria/SKILL.md`
...
4. Preserve any user instruction supplied after `/decision-criteria` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/decision-criteria`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/decision-criteria`
