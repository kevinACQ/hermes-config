---
name: eval
description: Universal build-test-review framework for any Claude Code task. Adds per-step success criteria, structured verification, and cross-session persistence to prevent half-built bridges. Two entry points: /eval plan (define criteria before building) and /eval check (verify current phase against criteria). Use when starting any multi-step task, when Kevin says 'let's plan this', 'eval this', 'what are the success criteria', or when a task has phases that need verification. Proactively suggest when a task is Standard or Heavy tier (see scaling below).
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Eval Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `eval`.
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/eval/SKILL.md`
...
4. Preserve any user instruction supplied after `/eval` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/eval`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/eval`
