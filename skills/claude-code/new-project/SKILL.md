---
name: new-project
description: Applies when the user wants to bootstrap a new code project from scratch and no directory at /Users/kevin/projects/<name>/ exists yet. Creates the project dir, scaffolds README/.gitignore/.env.example, initializes git, creates a private GitHub repo at kevinACQ/<name>, pushes the initial commit, and appends the project to ~/.claude/memory/active-projects.md.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# New Project Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `new-project`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/new-project <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/new-project/SKILL.md`
...
4. Preserve any user instruction supplied after `/new-project` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/new-project`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/new-project`
