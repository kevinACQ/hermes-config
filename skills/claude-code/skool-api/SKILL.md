---
name: skool-api
description: Read and interact with Skool communities via internal API — no browser required. Callable from other skills. Invoke before any task that reads a Skool post URL or needs community post data.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Skool Api Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `skool-api`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/skool-api <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-api/SKILL.md`
...
4. Preserve any user instruction supplied after `/skool-api` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-api`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-api`
