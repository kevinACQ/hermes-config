...
description: Draft and post the weekly Tuesday Training with Alex announcement to the Skool test community. Handles topic lookup, Zoom link extraction, post drafting, and email blast. Use when scheduling a Tuesday Training event post.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Tuesday Training Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `tuesday-training`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/tuesday-training <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/tuesday-training/SKILL.md`
...
4. Preserve any user instruction supplied after `/tuesday-training` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/tuesday-training`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/tuesday-training`
