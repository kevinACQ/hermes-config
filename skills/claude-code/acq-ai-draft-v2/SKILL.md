---
name: acq-ai-draft-v2
description: Generate an ACQ AI-authored advisor response for a VIP Skool community question using the CT framework (Context → Hypothesis → Evidence → Impact). Takes a Skool post URL, scrapes content, queries ACQ AI, strips citations, unsloppifies output, verifies completeness, and delivers to Kevin's Slack DM. ACQ AI is the author; Claude is the processor.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Acq Ai Draft V2 Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `acq-ai-draft-v2`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/acq-ai-draft-v2 <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-ai-draft-v2/SKILL.md`
...
4. Preserve any user instruction supplied after `/acq-ai-draft-v2` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-ai-draft-v2`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-ai-draft-v2`
