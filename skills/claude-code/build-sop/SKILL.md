---
name: build-sop
description: Standard operating procedure for every build — 8-step sequence from idea validation to lesson capture. Invoke at session start when Kevin says 'let's build', 'I want to build', or describes a new feature, automation, or project.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Build Sop Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `build-sop`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/build-sop <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/build-sop/SKILL.md`
...
4. Preserve any user instruction supplied after `/build-sop` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/build-sop`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/build-sop`
