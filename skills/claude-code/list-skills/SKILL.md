---
name: list-skills
description: Context-aware skill recommender. Analyzes the current session and surfaces the single best skill sequence to use next. Type /list-skills for a recommendation, /list-skills all for the full catalog.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# List Skills Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `list-skills`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/list-skills <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/list-skills/SKILL.md`
...
4. Preserve any user instruction supplied after `/list-skills` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/list-skills`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/list-skills`
