---
name: fathom-sync
description: Invoke Kevin's Claude Code `fathom-sync` skill from the symlink mirror.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Fathom Sync Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `fathom-sync`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/fathom-sync <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/fathom-sync/SKILL.md`
...
4. Preserve any user instruction supplied after `/fathom-sync` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/fathom-sync`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/fathom-sync`
