---
name: commit-skills
description: Applies when the user's stated goal is to commit or push local changes to skill files or CLAUDE.md in ~/.claude/ to cog-config.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Commit Skills Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `commit-skills`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/commit-skills <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/commit-skills/SKILL.md`
...
4. Preserve any user instruction supplied after `/commit-skills` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/commit-skills`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/commit-skills`
