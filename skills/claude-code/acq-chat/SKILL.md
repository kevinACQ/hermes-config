---
name: acq-chat
description: Chat with the ACQ AI Business Advisor (ai.acquisition.com) programmatically. Use when asked to query the Hormozi AI advisor, ask business strategy questions to ACQ AI, or interact with the acquisition.com chatbot.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Acq Chat Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `acq-chat`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/acq-chat <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-chat/SKILL.md`
...
4. Preserve any user instruction supplied after `/acq-chat` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-chat`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-chat`
