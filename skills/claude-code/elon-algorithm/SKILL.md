---
name: elon-algorithm
description: Apply Elon Musk's 5-step algorithm as a review lens to any requirement, plan, design, or feature — question requirements, delete, simplify, accelerate, automate, strictly in that order. Use to cut over-build and scope before committing, or as one opinion in a planning-phase review panel.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Elon Algorithm Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `elon-algorithm`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/elon-algorithm <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/elon-algorithm/SKILL.md`
...
4. Preserve any user instruction supplied after `/elon-algorithm` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/elon-algorithm`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/elon-algorithm`
