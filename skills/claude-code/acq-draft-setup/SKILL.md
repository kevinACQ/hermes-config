---
name: acq-draft-setup
description: One-time setup for acq-ai-draft-v1, acq-ai-draft-v2, and acq-ai-draft-kevins-v1. Builds the GStack binary, configures the user's Slack DM ID, and verifies Skool auth and ACQ Portal access. Run this once after cloning cog-config.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Acq Draft Setup Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `acq-draft-setup`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/acq-draft-setup <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-draft-setup/SKILL.md`
...
4. Preserve any user instruction supplied after `/acq-draft-setup` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-draft-setup`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/acq-draft-setup`
