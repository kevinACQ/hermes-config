---
name: skool-post-edit
description: Edit an existing Skool post — change title or replace body text. Use when fixing typos, updating links, or correcting content in a post that's already published. Does NOT send emails (email toggle is create-time only).
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Skool Post Edit Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `skool-post-edit`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/skool-post-edit <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-post-edit/SKILL.md`
...
4. Preserve any user instruction supplied after `/skool-post-edit` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-post-edit`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/skool-post-edit`
