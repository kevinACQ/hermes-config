---
name: remotion-video-ad
description: Invoke Kevin's Claude Code `remotion-video-ad` skill from the symlink mirror.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
# Remotion Video Ad Claude Code Skill Wrapper

## Purpose

This is a Hermes-native slash-command wrapper for Kevin's Claude Code skill `remotion-video-ad`.
Claude remains the source of truth. This wrapper exists so Kevin can invoke the workflow in Hermes as:

```text
/remotion-video-ad <task or target>
```
...
1. Immediately read the source Claude skill:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/remotion-video-ad/SKILL.md`
...
4. Preserve any user instruction supplied after `/remotion-video-ad` as the task target.
5. If the Claude skill references supporting files under `references/`, `templates/`, `scripts/`, or `assets/`, resolve them relative to:
   - `/Users/kevin/.hermes/external-repos/claude-code-skills/remotion-video-ad`
...
- Claude skill directory: `/Users/kevin/.hermes/external-repos/claude-code-skills/remotion-video-ad`
