...
§
Hermes/voice project: Alex voice is good enough. Hermes V3 Retell agent is live/verified as of 2026-05-24: agent `agent_5fd02629f0ee9491a55d6f87bb`, LLM `llm_60eb98cc38fb9e8e4e4b370cf7e1`, phone `+17159998518`, active eval/memory Sheet `1PxI279CzBfkb96ChL3vpCP9V8eaeiytOAhciQqmqgTc`. Never repeat Retell API keys in chat.
§
...
§
GBrain on Kevin's macOS: if CLI/MCP fails with a PGLite WASM runtime error, first test a temp PGLite brain. On 2026-05-08 the fix was to backup/rebuild ~/.hermes/.gbrain/brain.pglite from /Users/kevin/.hermes/brain markdown and run gbrain apply-migrations. Local embeddings now use Ollama nomic-embed-text at 768 dimensions with 100% embed coverage.
§
Kevin's MacBook has Ollama installed user-space at ~/Applications/Ollama.app, not /Applications, with CLI symlink ~/.local/bin/ollama; Ollama serves on localhost:11434 and has nomic-embed-text pulled. GBrain local PGLite uses ollama:nomic-embed-text at 768 dimensions with 100% embed coverage. Avoid Ollama prompts to move/update into /Applications because work laptop lacks admin access.